...
function resetAllStates() {
    devices.forEach(d => { d.isPowered = false; if(d.terminals) d.terminals.forEach(t => { t.isLive = false; t.isGnd = false; }); });
...
function run電位走査(mainPower, startIndex, keyProp) {
    devices.forEach(d => { if(!d.terminals || d.terminals.length === 0) d.terminals = Array.from({ length: 20 }, () => ({ isLive: false, isGnd: false })); });
    let visited = new Set(), queue = [{ deviceId: mainPower.id, terminalIndex: startIndex }];
    ...
    while (queue.length > 0) {
        ...
        if (currentDevice.terminals[curr.terminalIndex]) currentDevice.terminals[curr.terminalIndex][keyProp] = true;
        ...
        if (currentDevice.type === 'terminal_block' || (currentDevice.type === 'breaker' && currentDevice.isON)) {
            ...
            if (curr.terminalIndex === 11 || curr.terminalIndex === 12 || curr.terminalIndex === 13) reachableLocalTerminals.push(curr.terminalIndex);
        }
            ...
            if (curr.terminalIndex === 0 || curr.terminalIndex === 1) reachableLocalTerminals.push(curr.terminalIndex);
        }
        else if (['pilot_lamp', 'buzzer', 'analog_meter', 'digital_controller', 'panel_timer'].includes(currentDevice.type)) {
            reachableLocalTerminals.push(curr.terminalIndex === 0 ? 1 : 0);
        }
        reachableLocalTerminals.forEach(tIdx => { if (currentDevice.terminals[tIdx]) currentDevice.terminals[tIdx][keyProp] = true; queue.push({ deviceId: currentDevice.id, terminalIndex: tIdx }); });
        wires.forEach(w => {
...
function runSequenceSimulation() {
    ...
    for (let syncLoop = 0; syncLoop < 4; syncLoop++) {
        devices.forEach(d => { if(d.terminals) d.terminals.forEach(t => { t.isLive = false; t.isGnd = false; }); });
        run電位走査(mainPower, 0, 'isLive'); run電位走査(mainPower, 2, 'isGnd');
        devices.forEach(d => {
            let isPoweredThisLoop = false;
            if (d.type === 'relay' || d.type === 'contactor' || d.type === 'contact_block' || ['pilot_lamp', 'buzzer', 'analog_meter', 'digital_controller', 'panel_timer'].includes(d.type)) {
                if (d.terminals && d.terminals[0]?.isLive && d.terminals[1]?.isGnd) isPoweredThisLoop = true;
                if (d.type === 'relay' && d.terminals && d.terminals[12]?.isLive && d.terminals[13]?.isGnd) isPoweredThisLoop = true;
            }
    ...
    devices.forEach(d => {
        if (d.terminals) d.terminals.forEach((t, idx) => {
            if (t.isLive && !t.isGnd) {
                ...
                else if (d.type === 'relay') { if (idx === 8) opp = 4; if (idx === 9) opp = 5; if (idx === 10) opp = 6; }
    ...
    let loads = 0; devices.forEach(d => { if (d.isPowered && (d.type === 'relay' || d.type === 'contactor' || d.isLampElement || ['pilot_lamp', 'buzzer', 'analog_meter', 'digital_controller', 'panel_timer'].includes(d.type))) loads++; });
