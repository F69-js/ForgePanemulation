...
function drawBezel(ctx, cx, cy) {
    ...
    ctx.strokeStyle = '#57606f'; ctx.lineWidth = 1; ctx.stroke(); ctx.shadowColor = 'transparent';
}
function drawDeviceNameplate(ctx, d) {
    ...
    ctx.fillStyle = '#f1f2f6'; ctx.fillRect(px, py, pW, pH); ctx.shadowColor = 'transparent';
    ctx.fillStyle = '#2f3542'; ctx.font = 'bold 8px sans-serif'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle';
    ctx.fillText(d.label || "SPARE", cx, py + pH / 2 + 0.5);
    ctx.strokeStyle = '#ced6e0'; ctx.lineWidth = 0.5; ctx.strokeRect(px, py, pW, pH); ctx.restore();
}
function drawSingleDevice(ctx, d) {
    ctx.save(); ctx.shadowColor = 'rgba(0,0,0,.3)'; ctx.shadowBlur = 8; ctx.shadowOffsetX = 3; ctx.shadowOffsetY = 4;
    const cx = d.x + d.width / 2, cy = d.y + d.height / 2;
    if (d.layer === 'exterior') {
        drawDeviceNameplate(ctx, d); drawBezel(ctx, cx, cy);
        if (d.type === 'switch' || d.type === 'lamp_switch') {
            const bg = ctx.createRadialGradient(cx - 4, cy - 4, 2, cx, cy, 18); bg.addColorStop(0, d.type === 'lamp_switch' ? '#7ed6df' : '#55efc4');
            ...
            const lg = ctx.createRadialGradient(cx - 3, cy - 3, 1, cx, cy, 18); lg.addColorStop(0, '#ff7675'); lg.addColorStop(0.7, d.color); lg.addColorStop(1, '#c0392b');
            ...
            ctx.fillStyle = d.type === 'lamp_selector' ? '#2980b9' : '#2c3e50'; ctx.beginPath(); ctx.arc(cx, cy, 18, 0, Math.PI * 2); ctx.fill();
            ctx.fillStyle = '#111417'; ctx.fillRect(cx - 5, cy - 20, 10, 30); ctx.fillStyle = d.type === 'lamp_selector' ? '#7ed6df' : '#fff'; ctx.fillRect(cx - 1.5, cy - 18, 3, 12);
            ...
            ctx.fillStyle = '#2d3436'; ctx.fillRect(cx - 2, cy - 10, 4, 20); ctx.beginPath(); ctx.arc(cx, cy, 4, 0, Math.PI*2); ctx.fill();
        } else if (d.type === 'buzzer') {
            ctx.fillStyle = '#2d3436'; ctx.beginPath(); ctx.arc(cx, cy, 18, 0, Math.PI * 2); ctx.fill(); ctx.strokeStyle = '#111'; ctx.lineWidth = 2;
        ...
        const grad = ctx.createLinearGradient(d.x, d.y, d.x + d.width, d.y); grad.addColorStop(0, d.color); grad.addColorStop(1, adjustColor(d.color, -20));
        ctx.fillStyle = grad; ctx.fillRect(d.x, d.y, d.width, d.height); ctx.shadowColor = 'transparent';
        if (d.type === 'terminal_block') {
            ctx.fillStyle = '#f1f2f6'; ctx.fillRect(d.x + 4, d.y + d.height / 2 - 6, d.width - 8, 12);
            ctx.fillStyle = '#2f3542'; ctx.font = 'bold 8px Arial'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(d.name, d.x + d.width / 2, d.y + d.height / 2);
        ...
        } else { ctx.fillStyle = '#fff'; ctx.font = 'bold 8px monospace'; ctx.textAlign = 'center'; ctx.fillText(d.name, d.x + d.width / 2, d.y + 14); }
...
export function drawAll(ctx, canvas, panelConfig, devices, wires, activeWiring, hoveredTerminal) {
    ...
    const rg = ctx.createLinearGradient(0, dinRail.y, 0, dinRail.y + dinRail.height); rg.addColorStop(0, '#bdc3c7'); rg.addColorStop(.3, '#fff'); rg.addColorStop(.7, '#95a5a6'); rg.addColorStop(1, '#7f8c8d');
    ctx.fillStyle = rg; ctx.fillRect(0, dinRail.y, canvas.width, dinRail.height); ctx.strokeStyle = '#7f8c8d'; ctx.strokeRect(0, dinRail.y + 4, canvas.width, dinRail.height - 8);
    if (doorOpenProgress > 0) {
        ctx.save(); ctx.globalAlpha = doorOpenProgress;
        devices.forEach(d => { if (d.layer === "interior") drawSingleDevice(ctx, d); }); drawWires(ctx, wires, activeWiring); ctx.restore();
    }
    if (doorOpenProgress < 1) {
        ctx.save(); let dw = canvas.width * (1 - doorOpenProgress);
        if (dw > 0) {
            const dg = ctx.createLinearGradient(0, 0, dw, 0); dg.addColorStop(0, '#b2bec3'); dg.addColorStop(1, '#dfe6e9');
            ctx.fillStyle = dg; ctx.fillRect(0, 0, dw, canvas.height); ctx.fillStyle = 'rgba(0,0,0,0.2)'; ctx.fillRect(dw - 10, 0, 10, canvas.height);
            if (dw > 120) {
                ctx.save(); ctx.fillStyle = '#111'; ctx.fillRect(30, 40, Math.min(dw - 60, canvas.width - 60), 60); ctx.fillStyle = '#fff'; ctx.font = 'bold 18px monospace';
                if (dw > 300) { ctx.fillText(`名称: ${panelConfig.name}`, 50, 75); ctx.font = 'bold 14px monospace'; ctx.textAlign = 'right'; ctx.fillText(`SYSTEM: ${panelConfig.phase}`, dw - 50, 75); }
                ctx.restore();
            ...
            devices.forEach(d => { if (d.layer === "exterior") drawSingleDevice(ctx, d); }); ctx.restore();
        }
        ctx.restore();
    }
...
function drawWires(ctx, wires, activeWiring) {
    wires.forEach(w => {
        ...
        const cY = Math.max(s.y, e.y) + 40; ctx.beginPath(); ctx.moveTo(s.x, s.y); ctx.bezierCurveTo(s.x, cY, e.x, cY, e.x, e.y); ctx.stroke(); ctx.restore();
    });
    if (activeWiring) {
        const s = activeWiring.fromNode.getTerminalCoords(activeWiring.fromTerminal);
        ctx.save(); ctx.strokeStyle = 'rgba(231, 76, 60, 0.6)'; ctx.lineWidth = 3; ctx.beginPath(); ctx.moveTo(s.x, s.y); ctx.lineTo(activeWiring.currentX, activeWiring.currentY); ctx.stroke(); ctx.restore();
    }
}
