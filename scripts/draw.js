...
function drawSingleDevice(ctx, d) {
    ...
    const w = Number.isFinite(d.width) ? d.width : 60;
    const h = Number.isFinite(d.height) ? d.height : 60;
    const cx = x + w / 2, cy = y + h / 2;

    if (d.layer === 'exterior') {
        const pW = 54, pH = 14, px = cx - pW / 2, py = y - pH - 2; ctx.fillStyle = '#f1f2f6'; ctx.fillRect(px, py, pW, pH);
        ...
        if (d.type === 'terminal_block') {
            ctx.fillStyle = '#f1f2f6'; ctx.fillRect(x + 4, y + h / 2 - 6, w - 8, 12); ctx.fillStyle = '#2f3542'; ctx.font = 'bold 8px Arial'; ctx.textAlign = 'center'; ctx.fillText(d.name, x + w / 2, y + h / 2);
