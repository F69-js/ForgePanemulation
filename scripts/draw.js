...
const renderList = [
    ...
    /* 12: emergency_stop ★【新星】画像通りのキノコ型非常停止大ボタン */ (ctx, cx, cy) => {
        ...
        ctx.fillStyle=bg; ctx.beginPath(); ctx.arc(cx,cy,24,0,Math.PI*2); ctx.fill(); // 大口径キノコヘッド
        ctx.strokeStyle='rgba(0,0,0,0.4)'; ctx.lineWidth=2; ctx.stroke();
        ctx.strokeStyle='rgba(255,255,255,0.3)'; ctx.lineWidth=1.5; ctx.beginPath(); ctx.arc(cx,cy,14,Math.PI*0.1,Math.PI*0.8); ctx.stroke(); // 矢印のガイド円弧穴
    }
];
...
function drawSingleDevice(ctx, d) {
    ...
    if (d.layer === 'exterior') {
        ...
        if (isRoundBody) {
            ...
            ctx.fillStyle = '#2d3436'; ctx.fillRect(cx - 15, cy - 15, 30, 30); // 中央黒本体ホルダー
            // ★【新仕様】非常停止ボタンの裏側なら、青・赤・黄（ランプソケット）がトリプルで合体している写真を完全再現！
            if (d.extraConfig?.isEMO) {
                ctx.fillStyle = '#2980b9'; ctx.fillRect(cx - 22, cy - 25, 14, 10); // 左側：青NO
                ctx.fillStyle = '#e74c3c'; ctx.fillRect(cx + 8,  cy - 25, 14, 10); // 右側：赤NC
                ctx.fillStyle = '#f1c40f'; ctx.fillRect(cx - 10, cy + 15, 20, 10); // 下側：黄色ソケット
            } else {
