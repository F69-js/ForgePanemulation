...
function drawSingleDevice(ctx, d) {
    ...
    if (d.layer === 'exterior') {
        ...
        if (!['analog_meter', 'digital_controller', 'panel_timer'].includes(d.type)) drawBezel(ctx, cx, cy);
        if (d.typeIndex !== undefined && renderList[d.typeIndex]) renderList[d.typeIndex](ctx, cx, cy, d);
    } else {
        // ★【新仕様】メーター、ブザー、タイマー、調節計の裏面を「リアルな丸胴体」で描画する条件分岐
        const isRoundBody = ['contact_block', 'analog_meter', 'digital_controller', 'panel_timer'].includes(d.type) || (d.type === 'contact_block' && d.isLampElement);
        if (isRoundBody) {
...
export function drawAll(ctx, canvas, panelConfig, devices, wires, activeWiring, hoveredTerminal, dinRails = []) {
    ...
    if (doorOpenProgress < 1) {
        ctx.save(); let dw = canvas.width * (1 - doorOpenProgress); if (dw > 0) {
            ...
            // ★【新仕様】一番上の分電盤名称プレートを白アクリルベース＋リアルな彫刻影に大改造（コンパクト化）
            if (dw > 120) {
                const mx = Math.min(dw / 2, canvas.width / 2), pW = Math.min(dw - 100, 240), pH = 26, px = mx - pW / 2, py = 40;
