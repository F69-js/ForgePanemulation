...
function drawBezel(ctx, cx, cy) { ctx.fillStyle = '#1e252b'; ctx.beginPath(); ctx.arc(cx, cy, 26, 0, Math.PI * 2); fill(); const mg = ctx.createLinearGradient(cx - 26, cy - 26, cx + 26, cy + 26); mg.addColorStop(0, '#ffffff'); mg.addColorStop(0.5, '#7f8c8d'); mg.addColorStop(1, '#2c3e50'); ctx.strokeStyle = mg; ctx.lineWidth = 2.5; ctx.stroke(); }
...
const renderList = [
    ...
    /* 3: selector_sw */ (ctx, cx, cy) => { ctx.fillStyle='#111417'; ctx.beginPath(); ctx.arc(cx, cy, 18, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = '#2d3436'; ctx.fillRect(cx - 6, cy - 20, 12, 34); ctx.fillStyle = '#ffffff'; ctx.fillRect(cx - 2, cy - 18, 4, 14); },
    /* 4: lamp_selector */ (ctx, cx, cy, d) => { const lg = ctx.createRadialGradient(cx-3,cy-3,1,cx,cy,18); lg.addColorStop(0,adj(d.color,40)); lg.addColorStop(1,adj(d.color,-20)); ctx.fillStyle=lg; ctx.beginPath(); ctx.arc(cx,cy,18,0,Math.PI*2); ctx.fill(); ctx.fillStyle='#111417'; ctx.fillRect(cx-7,cy-20,14,34); ctx.fillStyle=adj(d.color,50); ctx.fillRect(cx-4,cy-18,8,30); },
    /* 5: key_switch */ (ctx, cx, cy) => { const kg = ctx.createLinearGradient(cx-15,cy-15,cx+15,cy+15); kg.addColorStop(0,'#dfe6e9'); kg.addColorStop(0.5,'#b2bec3'); kg.addColorStop(1,'#636e72'); ctx.fillStyle=kg; ctx.beginPath(); ctx.arc(cx,cy,18,0,Math.PI*2); ctx.fill(); ctx.fillStyle='#2d3436'; ctx.fillRect(cx-2,cy-10,4,20); ctx.beginPath(); ctx.arc(cx,cy,4,0,Math.PI*2); ctx.fill(); },
    /* 6: buzzer */ (ctx, cx, cy, d) => { ctx.fillStyle='#2d3436'; ctx.fillRect(d.x, d.y, 60, 60); ctx.fillStyle='#1e252b'; ctx.beginPath(); ctx.arc(cx,cy,20,0,Math.PI*2); ctx.fill(); ctx.strokeStyle='#111'; ctx.lineWidth=3; for(let i=-2;i<=2;i++){ ctx.beginPath(); ctx.moveTo(cx-14,cy+i*5); ctx.lineTo(cx+14,cy+i*5); ctx.stroke(); } },
    ...
    /* 12: emergency_stop (キノコ型・ターンリセット白矢印完全復元版) */ (ctx, cx, cy) => {
        const bg = ctx.createRadialGradient(cx-5,cy-5,3,cx,cy,24); bg.addColorStop(0,'#ff7675'); bg.addColorStop(0.6,'#d63031'); bg.addColorStop(1,'#8b0000');
        ctx.fillStyle=bg; ctx.beginPath(); ctx.arc(cx,cy,24,0,Math.PI*2); ctx.fill();
        ctx.strokeStyle='rgba(0,0,0,0.4)'; ctx.lineWidth=2; ctx.stroke();
        // ★【完全復活】表面にターンリセット方向を示す綺麗でリアルな3方向の白矢印を描画！
        ctx.save(); ctx.strokeStyle='rgba(255,255,255,0.7)'; ctx.lineWidth=2; ctx.lineCap='round';
        for (let i = 0; i < 3; i++) {
            ctx.save(); ctx.translate(cx, cy); ctx.rotate((i * 120 * Math.PI) / 180);
            ctx.beginPath(); ctx.arc(0, 0, 14, -Math.PI*0.2, Math.PI*0.1); ctx.stroke(); // 円弧線
            ctx.fillStyle='rgba(255,255,255,0.7)'; ctx.beginPath(); ctx.moveTo(12, 4); ctx.lineTo(15, -2); ctx.lineTo(18, 5); ctx.fill(); // 矢印の三角頭
            ctx.restore();
        }
        ctx.restore();
    }
...
function drawSingleDevice(ctx, d) {
    ...
    if (d.layer === 'exterior') {
        ...
        if (isRoundBody) {
            ctx.fillStyle = '#2c3e50'; ctx.beginPath(); ctx.arc(cx, cy, w / 2 - 2, 0, Math.PI * 2); ctx.fill(); ctx.strokeStyle = '#1a252f'; ctx.lineWidth = 2; ctx.stroke();
        } else if (d.type === 'contact_block') {
            ctx.fillStyle = '#2d3436'; ctx.fillRect(cx - 15, cy - 15, 30, 30);
            if (d.extraConfig?.isEMO) {
                ctx.fillStyle = '#2980b9'; ctx.fillRect(cx - 22, cy - 25, 14, 10);
                ctx.fillStyle = '#e74c3c'; ctx.fillRect(cx + 8,  cy - 25, 14, 10);
                ctx.fillStyle = '#f1c40f'; ctx.fillRect(cx - 10, cy + 15, 20, 10);
            } else {
                ctx.fillStyle = d.color; ctx.fillRect(cx - 15, cy - 25, 30, 10); ctx.fillRect(cx - 15, cy + 15, 30, 10);
            }
        } else {
