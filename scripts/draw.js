...
const adjustColor = (hex, pct) => {
    if (hex.toLowerCase() === '#ffffff') return pct < 0 ? '#d2d7d9' : '#ffffff';
    let r = parseInt(hex.substring(1, 3), 16), g = parseInt(hex.substring(3, 5), 16), b = parseInt(hex.substring(5, 7), 16);
    r = Math.max(0, Math.min(255, parseInt(r * (100 + pct) / 100)));
    g = Math.max(0, Math.min(255, parseInt(g * (100 + pct) / 100)));
    b = Math.max(0, Math.min(255, parseInt(b * (100 + pct) / 100)));
...
function drawSingleDevice(ctx, d) {
    ...
    if (d.layer === 'exterior') {
        ...
        if (d.type === 'switch' || d.type === 'lamp_switch') {
            ...
            bg.addColorStop(0, d.type === 'lamp_switch' ? adjustColor(d.color, 40) : adjustColor(d.color, 20));
            bg.addColorStop(0.6, d.color); bg.addColorStop(1, adjustColor(d.color, -35));
            ...
            ctx.fillStyle = d.type === 'lamp_selector' ? adjustColor(d.color, 40) : '#fff'; ctx.fillRect(cx - 1.5, cy - 18, 3, 12);
...
function drawSingleRail(ctx, canvas, rail) {
    const rg = ctx.createLinearGradient(0, rail.y, 0, rail.y + rail.height);
    rg.addColorStop(0, '#bdc3c7'); rg.addColorStop(.3, '#fff'); rg.addColorStop(.7, '#95a5a6'); rg.addColorStop(1, '#7f8c8d');
    ctx.fillStyle = rg; ctx.fillRect(0, rail.y, canvas.width, rail.height);
    ctx.strokeStyle = '#7f8c8d'; ctx.lineWidth = 1; ctx.strokeRect(0, rail.y + 4, canvas.width, rail.height - 8);
}
