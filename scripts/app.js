...
let draggedDevice = null, draggedRail = null, offsetX = 0, offsetY = 0, activeWiring = null, hoveredTerminal = null;
...
document.getElementById('preview-mode-btn')?.addEventListener('click', (e) => {
    isPreviewMode = !isPreviewMode; const btn = e.target;
    if (isPreviewMode) { btn.innerText = "⏹️ プレビュー停止"; btn.style.background = "linear-gradient(135deg, #e67e22, #d35400)"; clearRightMenu(); } 
    else { btn.innerText = "▶️ プレビュー開始"; btn.style.background = "linear-gradient(135deg, #2ecc71, #27ae60)"; devices.forEach(d => d.isON = false); updateButtonStates(currentMode); }
    draw();
});
...
document.getElementById('build-btn')?.addEventListener('click', () => {
    panelConfig.name = document.getElementById('panel-name').value.toUpperCase(); const voltSelect = document.getElementById('panel-voltage');
...
document.getElementById('view-btn')?.addEventListener('click', () => { if (!isPreviewMode && toggleDoorMode(devices)) { updateButtonStates(currentMode); animateLoop(); } });
...
function handleMouseDown(e) {
    ...
    if (currentMode === "interior") {
        if (!activeWiring) {
            ...
            if (hitTD !== null) { if (hitTD !== activeWiring.fromNode) { wires.push({ fromNode: activeWiring.fromNode, fromTerminal: activeWiring.fromTerminal, toNode: hitTD, toTerminal: hitTI, points: [...activeWiring.points], color: '#e74c3c' }); activeWiring = null; draw(); } return; } 
