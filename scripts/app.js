...
function drawBezel(ctx, cx, cy) { ctx.fillStyle = '#1e252b'; ctx.beginPath(); ctx.arc(cx, cy, 26, 0, Math.PI * 2); ctx.fill(); ctx.strokeStyle = '#57606f'; ctx.lineWidth = 1; ctx.stroke(); }
...
function drawSingleDevice(ctx, d) {
    ...
    if (d.layer === 'exterior') {
        ...
        if (d.type === 'switch' || d.type === 'lamp_switch') {
            const bg = ctx.createRadialGradient(cx - 4, cy - 4, 2, cx, cy, 18); bg.addColorStop(0, d.type === 'lamp_switch' ? adjustColor(d.color, 40) : adjustColor(d.color, 20)); bg.addColorStop(0.6, d.color); bg.addColorStop(1, adjustColor(d.color, -35));
            ctx.fillStyle = bg; ctx.beginPath(); ctx.arc(cx, cy, 18, 0, Math.PI * 2); ctx.fill();
        } else if (d.type === 'pilot_lamp') {
            const lg = ctx.createRadialGradient(cx - 3, cy - 3, 1, cx, cy, 18); lg.addColorStop(0, adjustColor(d.color, 50)); lg.addColorStop(0.7, d.color); lg.addColorStop(1, adjustColor(d.color, -40));
            ctx.fillStyle = lg; ctx.beginPath(); ctx.arc(cx, cy, 18, 0, Math.PI * 2); ctx.fill(); ctx.strokeStyle = 'rgba(255,255,255,0.2)'; ctx.beginPath(); ctx.arc(cx, cy, 12, 0, Math.PI*2); ctx.stroke(); ctx.beginPath(); ctx.arc(cx, cy, 6, 0, Math.PI*2); ctx.stroke();
            ...
            ctx.fillStyle = '#111417'; ctx.fillRect(cx - 5, cy - 20, 10, 30); ctx.fillStyle = d.type === 'lamp_selector' ? adjustColor(d.color, 40) : '#fff'; ctx.fillRect(cx - 1.5, cy - 18, 3, 12);
            ...
            ctx.fillStyle = kg; ctx.beginPath(); ctx.arc(cx, cy, 18, 0, Math.PI * 2); ctx.fill(); ctx.fillStyle = '#2d3436'; ctx.fillRect(cx - 2, cy - 10, 4, 20); ctx.beginPath(); ctx.arc(cx, cy, 4, 0, Math.PI*2); ctx.fill();
        ...
        if (d.type === 'terminal_block') {
            ctx.fillStyle = '#f1f2f6'; ctx.fillRect(d.x + 4, d.y + d.height / 2 - 6, d.width - 8, 12); ctx.fillStyle = '#2f3542'; ctx.font = 'bold 8px Arial'; ctx.textAlign = 'center'; ctx.textBaseline = 'middle'; ctx.fillText(d.name, d.x + d.width / 2, d.y + d.height / 2);
...
function drawSingleRail(ctx, canvas, rail) {
    const rg = ctx.createLinearGradient(0, rail.y, 0, rail.y + dinRailHeight); rg.addColorStop(0, '#bdc3c7'); rg.addColorStop(.3, '#fff'); rg.addColorStop(.7, '#95a5a6'); rg.addColorStop(1, '#7f8c8d');
    ctx.fillStyle = rg; ctx.fillRect(0, rail.y, canvas.width, dinRailHeight); ctx.strokeStyle = '#7f8c8d'; ctx.lineWidth = 1; ctx.strokeRect(0, rail.y + 4, canvas.width, dinRailHeight - 8);
}
export function drawAll(ctx, canvas, panelConfig, devices, wires, activeWiring, hoveredTerminal, dinRails = []) {
    ctx.clearRect(0, 0, canvas.width, canvas.height); dinRails.forEach(rail => drawSingleRail(ctx, canvas, rail));
    if (doorOpenProgress > 0) {
        ctx.save(); ctx.globalAlpha = doorOpenProgress; devices.forEach(d => { if (d.layer === "interior") drawSingleDevice(ctx, d); }); drawWires(ctx, wires, activeWiring); ctx.restore();
    }
